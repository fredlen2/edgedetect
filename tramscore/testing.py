...
from tramscore.decoder import path_to_advertname
from tramscore import Tramscore
from tramscore.fingerprint import *
import traceback
import fnmatch
import os, re, ast
import subprocess
...
class TramscoreTest(object):
    def __init__(self, folder, seconds):
        super(TramscoreTest, self).__init__()
    ...
    def get_line_id(self, advert):
        for i, s in enumerate(self.test_adverts):
            if advert == s:
                return i
        self.test_adverts.append(advert)
        return len(self.test_adverts) - 1
    ...
    def create_plots(self, name, results, results_folder):
        for sec in range(0, len(self.test_seconds)):
            ...
            for x in range(0, self.n_lines):
                labels[x] = "advert %s" % (x+1)
            ax.set_xticklabels(labels)

            box = ax.get_position()
            ax.set_position([box.x0, box.y0, box.width * 0.75, box.height])

            #ax.legend( (rects1[0]), ('Trmscore'), loc='center left', bbox_to_anchor=(1, 0.5))
    ...
    def begin(self):
        for f in self.test_files:
            ...
            advert = path_to_advertname(f).split("_")[0]
            line = self.get_line_id(advert)
            ...
            if result.strip() == "None":
                ...
                # which advert did we predict?
                result = ast.literal_eval(result)
                advert_result = result["advert_name"]
                log_msg('advert: %s' % advert)
                log_msg('advert_result: %s' % advert_result)

                if advert_result != advert:
                    ...
                    self.result_query_duration[line][col] = round(result[Tramscore.MATCH_TIME], 3)
                    self.result_match_confidence[line][col] = result[Tramscore.CONFIDENCE]

                    advert_start_time = re.findall("\_[^\_]+",f)
                    advert_start_time = advert_start_time[0].lstrip("_ ")

                    result_start_time = round((result[Tramscore.OFFSET] * DEFAULT_WINDOW_SIZE *
                                               DEFAULT_OVERLAP_RATIO) / (DEFAULT_FS), 0)

                    self.result_matching_times[line][col] = int(result_start_time) - int(advert_start_time)
                    if (abs(self.result_matching_times[line][col]) == 1):
                        self.result_matching_times[line][col] = 0

                    log_msg('query duration: %s' % round(result[Tramscore.MATCH_TIME], 3))
                    log_msg('confidence: %s' % result[Tramscore.CONFIDENCE])
                    log_msg('advert start_time: %s' % advert_start_time)
